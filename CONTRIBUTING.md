# Contributing to FSharp.Analyzers.SDK
...
Commercial support is provided by [Lambda Factory](http://lambdafactory.io/) and other companies specializing in F# consulting.
...
your Operating System, as well as your `dotnet` (or `mono` \ .Net Framework), F# and FSharp.Analyzers.SDK versions. Please provide steps to
...
1. [Fork](https://help.github.com/articles/fork-a-repo/) the project, clone your fork,
   ...
   git clone https://github.com/<your-username>/FSharp.Analyzers.SDK
   # Navigate to the newly cloned directory
   cd FSharp.Analyzers.SDK
   # Assign the original repo to a remote called "upstream"
   git remote add upstream https://github.com/Krzysztof-Cieslak/FSharp.Analyzers.SDK
   ```
...
A new release happens automatically when the version in the `CHANGELOG.md` was increased and the code is on the `main` branch.
We verify the version isn't published on NuGet yet, and if that is the case, we push the packages to NuGet and create a new GitHub release.
...
2. Create new commit and push it to the repo
    1. `git add CHANGELOG.md`
    1. `git commit -m "changelog for v0.45.0"`
    1. `git push [insert-remote] main`

You can preview the release locally without pushing anything:
